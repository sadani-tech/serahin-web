...
export function SellerAuthForm({ mode, token = "" }: { mode: "login" | "activate"; token?: string }) {
  ...
  return <form action={action} className="mt-6 space-y-4 rounded-2xl border border-sand-200 bg-white p-6 shadow-sm">
    ...
    <button disabled={pending || (mode === "activate" && !token)} className="min-h-11 w-full rounded-xl bg-brand-700 px-5 font-extrabold text-white">{pending ? "Memproses…" : mode === "login" ? "Masuk sebagai Seller" : "Aktifkan akun Seller"}</button>
  </form>;
}
