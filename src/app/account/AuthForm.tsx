...
export function AuthForm({ action, fields, hidden, submit, footer, success }: { action: Action; fields: Field[]; hidden?: Record<string, string>; submit: string; footer?: { href: string; label: string }; success?: { title: string; copy: string } }) {
  ...
  if (state?.message && success) return <div role="status" className="space-y-5 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-brand-100 text-2xl text-brand-700">✓</div><div><h2 className="text-lg font-extrabold text-sand-900">{success.title}</h2><p className="mt-2 text-sm leading-6 text-sand-600">{success.copy}</p></div><p className="rounded-xl bg-sun-50 p-3 text-left text-sm font-bold text-sun-800">{state.message}</p><Link href="/account/login" className="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-brand-600 px-4 text-sm font-extrabold text-white">Kembali ke halaman masuk</Link></div>;
