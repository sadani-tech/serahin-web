...
export function CartPageClient() {
  const { cart, ready, authenticated, syncConflict, setQuantity, removeItem, clear, sync } = useCart();
  ...
  return <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
    <section>
      ...
      {syncConflict && <div className="mt-5 rounded-2xl border border-sun-300 bg-sun-50 p-4 text-sm text-sun-900"><p className="font-extrabold">Keranjang akun berisi Batch PO lain.</p><p className="mt-1">Saat checkout, Anda dapat menggantinya dengan isi perangkat ini. Tidak ada pesanan yang terhapus.</p></div>}
      <div className="mt-6 space-y-3">{cart.items.map((item) => <article key={`${item.variantId}:${item.selectedColor ?? ""}`} className="flex gap-4 rounded-2xl border border-sand-200 bg-white p-4 shadow-sm">
        ...
        <div className="min-w-0 flex-1"><h2 className="font-extrabold text-sand-900">{item.name}</h2>{item.selectedColor && <p className="text-sm text-sand-500">Warna: {item.selectedColor}</p>}<p className="mt-1 font-extrabold text-brand-700">{formatRupiah(item.price)}</p>
    ...
    <aside className="h-fit rounded-3xl border border-sand-200 bg-white p-6 shadow-sm lg:sticky lg:top-24">
      ...
      <label className="mt-5 flex gap-3 text-sm leading-5 text-sand-700"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} className="mt-1" /><span>Saya menyetujui <Link href="/terms" className="font-bold text-brand-700 underline">Syarat & Ketentuan</Link>, Kebijakan Privasi, dan Kebijakan Refund.</span></label>
      ...
      <button type="button" disabled={!accepted || pending} onClick={checkout} className="mt-5 min-h-12 w-full rounded-xl bg-brand-600 px-5 text-sm font-extrabold text-white disabled:bg-sand-300">{pending ? "Memproses…" : authenticated ? "Lanjutkan Checkout" : "Masuk untuk Checkout"}</button>
