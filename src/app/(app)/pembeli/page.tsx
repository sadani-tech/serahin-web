...
async function AdminBuyerList({ sp }: { sp: Params }) {
  ...
  return <div className="space-y-6"><header><p className="text-xs font-extrabold uppercase tracking-[.16em] text-brand-700">Admin internal</p><h1 className="mt-1 text-3xl font-extrabold text-sand-900">Pembeli</h1><p className="mt-1 text-sm text-sand-500">Kelola akun Buyer, profil, dan histori pembelian lintas Seller.</p></header>
    <form className="grid gap-3 rounded-2xl border border-sand-200 bg-white p-4 md:grid-cols-2 xl:grid-cols-[minmax(16rem,1fr)_auto_auto_auto_auto]"><input name="search" defaultValue={sp.search} placeholder="Cari nama, email, atau telepon" className="min-h-11 rounded-xl border border-sand-300 px-3"/><select name="status" defaultValue={sp.status ?? ""} className="min-h-11 rounded-xl border border-sand-300 px-3"><option value="">Semua status</option><option value="ACTIVE">Aktif</option><option value="INACTIVE">Nonaktif</option></select><select name="sort" defaultValue={sp.sort ?? "createdAt"} aria-label="Urutkan pembeli" className="min-h-11 rounded-xl border border-sand-300 px-3"><option value="createdAt">Tanggal bergabung</option><option value="name">Nama</option><option value="lastLoginAt">Login terakhir</option><option value="updatedAt">Pembaruan terakhir</option></select><select name="order" defaultValue={sp.order === "asc" ? "asc" : "desc"} aria-label="Arah urutan" className="min-h-11 rounded-xl border border-sand-300 px-3"><option value="desc">Terbaru/Z-A</option><option value="asc">Terlama/A-Z</option></select><div className="flex gap-2"><select name="limit" defaultValue={sp.limit ?? "20"} aria-label="Jumlah data per halaman" className="min-h-11 rounded-xl border border-sand-300 px-3"><option value="20">20/baris</option><option value="50">50/baris</option><option value="100">100/baris</option></select><button className="min-h-11 rounded-xl bg-brand-600 px-5 font-bold text-white">Terapkan</button></div></form>
