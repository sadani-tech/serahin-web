...
export function ShopeeCheckoutActions({
  ...
  return <form action={formAction} className="mt-3 grid gap-2 rounded-lg border border-orange-200 bg-orange-50 p-3 text-sm sm:grid-cols-3">
    ...
    <select name="keputusan" value={decision} onChange={(event) => setDecision(event.target.value as "VERIFIED" | "REJECTED")} className="min-h-10 rounded-lg border border-orange-300 bg-white px-2 font-semibold">
