...
export default async function VerificationPage({ searchParams }: { searchParams: Promise<{ campaignId?: string; variantId?: string; page?: string }> }) {
  ...
  return <div className="space-y-6">
    <div><h1 className="text-2xl font-bold text-sand-900">Antrian Verifikasi</h1><p className="mt-1 text-sm text-sand-500">Tinjau pesanan baru dari seluruh Batch PO. Pesanan mencurigakan ditampilkan lebih dahulu.</p></div>
    <Card className="p-4"><form className="flex flex-wrap items-end gap-3">
      ...
      <label className="text-sm">Varian<Select name="variantId" defaultValue={sp.variantId ?? ""} className="mt-1 block"><option value="">Semua varian</option>{selectedCampaign?.variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.namaVarian}</option>)}</Select></label>
      <button className="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-bold text-white shadow-brand transition-all hover:bg-brand-700 active:translate-y-px">Terapkan</button>
