...
export default async function VerificationPage({ searchParams }: { searchParams: Promise<{ campaignId?: string; variantId?: string; page?: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-4"><form className="flex flex-wrap items-end gap-3">
      <label className="text-sm">Batch PO<select name="campaignId" defaultValue={sp.campaignId ?? ""} className="mt-1 block rounded-lg border border-sand-300 px-3 py-2"><option value="">Semua Batch PO</option>{campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.namaProduk}</option>)}</select></label>
